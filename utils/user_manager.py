...
class User:
    ...
    def get_reminders(self):
        ...
        data = [{columns[i]: reminder[i] for i in range(len(reminder))} for reminder in reminders]
...
class Users:
    ...
    def add_user(self, user: User):
        ...
        if not self.get_user(user.telegram_id):
            ...
            self.db.insert("users", values=list(to_add.values()), columns=list(to_add.keys()))
            self.db.conn.close()
            return True
        return False
